...
#source("Y:/INSHORE SCALLOP/Survey/2017/temperature/Extract_survey_temperatures_function.r")
#survey.bottom.temps(survey_time = "start",export=T)

survey.bottom.temps <- function(direct = "Y:/Offshore/Assessment/", year = 2023, cruise = "LE18", bank = "GBa",
                                survey_time= "start",num.temps = 8,tow_duration = 20,fig="pdf",export=F)
{
  ...
  ## read in files
  towdata <- read.csv(paste0(direct, "/Data/Survey_data/", year, "/Database loading/", cruise, "/", bank, "/", "OS.scallop.tow.",bank, cruise, ".csv"))
  # Fix column names and put tow number back in
  names(towdata)[2] <- "Tow"
  # Now pull in the minilog files which contain the temperature by time files.
  #read in minilog files
  temps<- list.files(path = paste0(direct, "/Data/Survey_data/", year, "/Database loading/temperature/"), pattern=".csv", full=TRUE)
  ...
  if(survey_time %in% c("both","start"))
  {
    ...
    towdata$Tow_date <- str_pad(towdata$Tow_date,8,pad="0")
    # Now stitch the time and data together to get a Date-Time column
    # Tell R that the tow data is in UTC
    towdata$Datetime_UTC_start <- as.POSIXct(strptime(paste0(towdata$Tow_date,towdata$Time_start.UTC),format = "%d%m%Y%H%M",tz = "UTC"))
  ...
  {
    # Now do the same thing for the end times...
    towdata$Time_end.UTC <- str_pad(towdata$Time_end.UTC,4,pad="0")
    # Now stictch the time and data together to get a Date-Time column
    towdata$Datetime_UTC_end <- as.POSIXct(strptime(paste0(towdata$Tow_date_end,towdata$Time_end.UTC),format = "%d%m%Y%H%M",tz = "UTC"))
    ...
    # User defined tow end time.
    if(survey_time == "end") towdata$Datetime_local_start <- towdata$Datetime_local_end - lubridate::minutes(tow_duration)
    # Only use the end time for ordering if running end time only..
    if(survey_time == "end") towdata <- towdata %>% arrange(Datetime_local_end)
  } # if(survey_time %in% c("both","end"))
  ...
  if(fig == "pdf") pdf(paste(direct,"temperature/",cruise,"-Tow_",survey_time,"_time_vs_temp_profile.pdf",sep=""),onefile=T,width=11,height=6)
  ...
  if(export == T) write.csv(tempsfinal, paste0(direct, "temperature/", cruise, "_towtemperatures.csv"))
} # end function